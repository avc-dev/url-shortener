// Package handler реализует HTTP-хендлеры сервиса сокращения URL.
// Каждый хендлер принимает запрос, делегирует бизнес-логику в URLUsecase
// и опционально уведомляет систему аудита об операции.
package handler
